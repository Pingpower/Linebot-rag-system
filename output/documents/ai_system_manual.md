...
1. **引導式對話與彈性引導層數**：
   ...
   - 系統建議並傾向將核心問題在 3 層對話內引導回答完畢，以維持對話的簡潔與好觀感。
...
3. **本地模型一鍵熱切換與一鍵刪除**
   ...
   - **一鍵啟用**：點選非運行中模型旁的「一鍵啟用」，後端會自動：
     - 修改 `~/.config/systemd/user/linebot-llama.service` 中的模型檔案路徑。
     - 執行 `systemctl --user daemon-reload`，重啟 `linebot-llama` 服務，實現無縫切換。
   - **一鍵刪除**：針對非目前啟用中的模型，列表會提供紅色的 **「🗑️ 刪除」** 按鈕。
     - 點擊並於瀏覽器視窗防呆確認後，後端將自動且安全地移除該 `.gguf` 檔案以即時釋放硬碟空間。
     - 系統會自動阻擋刪除「當前執行中」的模型，以保障本地 LLaMA 引擎的穩定性。
